...
const logger = new logger_1.Logger('TransactionProcessor');
async function processTransaction(rawTransaction) {
    try {
...
function extractTransactionDetails(raw) {
    try {
        ...
        switch (programId) {
            ...
            case 'metaqbxxUerdq28cj1RbAWkYQm3ybzjb6a8bt518x1s': // Metaplex Token Metadata
                if (raw.meta?.innerInstructions?.some((ix) => ix.instructions.some((i) => i.programId === 'TokenkegQfeZyiNwAJbNbGKPFXCWuBvf9Ss623VQ5DA'))) {
                    type = 'NFT_MINT';
                }
                break;
